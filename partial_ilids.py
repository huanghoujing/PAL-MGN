...
class PartialiLIDs(Dataset):
    ...
    def camera(file_path):
        ...
        :return: camera id
        """
        cam = 0 if 'Probe' in file_path else 1
        return cam
    ...
    def _get_im_paths(self, split):
        if split == 'query':
            im_paths = sorted(list_pictures(osp.join(self.root, 'Partial_iLIDS/Probe')))
        elif split == 'gallery':
            im_paths = sorted(list_pictures(osp.join(self.root, 'Partial_iLIDS/Gallery')))
        else:
    ...
    def __init__(self, transform=None, target_transform=None, loader=default_loader, training=None, use_kpt=False, use_ps=False, split='train'):
        ...
        self.ps_dir = osp.join(self.root, 'Partial_iLIDS_ps_label') if use_ps else None
